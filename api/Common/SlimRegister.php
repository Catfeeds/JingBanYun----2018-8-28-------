...
function HTTPGET($parm)
{
    return !empty($_GET["$parm"]) ? $_GET["$parm"] : null;
}
...
$authTeacherLogin = function () {
    $userId = HTTPGET('id');
    $token = HTTPGET('token');


    if ($userId == null || $token == null) {
        renderJsonResponse(401, "未授权访问", array());
        exit();
    }

    $user = getTeacherById($userId);
...
function renderJsonResponse($status, $message, $data, $taskPoints = 0)
{
    //if(empty($data) && !is_array($data))
    //$data = (object) $data;
    $result = json_encode(array("status" => $status, "message" => $message, "result" => $data, "task_points" => $taskPoints));
    $callback = HTTPGET("callback");
...
//获取用户信息
function getTeacherByTelephone($telephone)
{
    $db = new Database();
...
function sendEmail($address, $subject, $body)
{
    ...
    $smtpserverport = 25;
    $smtpusermail = "1481684223@qq.com";
    $smtpemailto = $address;
    $smtpuser = "1481684223@qq.com";
    $smtppass = "P@ssw0rd@126.com";
    $mailsubject = $subject;
    $mailbody = $body;
...
function sendJPush($content, $alias, $tags, $m_type, $m_value, $m_object)
{
    return;
    $pushObj = new Jpush();
    if (!empty($alias)) $receive = array("alias" => array($alias));
    if (!empty($tags)) $receive = array('tag' => array($tags));
    $pushObj->push($receive, $content, $m_type, $m_value, $m_object, 86400);
}
