...
$(document).on('click','.addExercise',function(){
    ...
    datalist['big_paper_id'] = big_paper_id;
    datalist['paper_id'] = paper_id;
    ...
    $.ajax({
        ...
        url: 'index.php?m=Exercise&c=CreateExercise&a=addCompositeExercise', //调用WebService的地址和方法名称组合---WsURL/方法名
        data:requestdata,
        beforeSend:function () {
...
function getXuanzhe(iframeid,type,topicNum) {
    ...
    var messageJx = $('#'+iframeid)[0].contentWindow.messageJx;
    messageJx = messageJx.getContent();
    var setAnswerinfo = $('#'+iframeid)[0].contentWindow.$('.setAnswerinfo').val();
...
function layerpaper(paperid) {
    ...
    $.NotifyBox.NotifyTwoCallTwo('添加成功','添加成功,是否继续进行习题添加？','继续','查看',function(){
