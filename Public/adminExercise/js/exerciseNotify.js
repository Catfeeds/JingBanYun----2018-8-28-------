(function() {
	$.NotifyBox = {
		...
		//两个按钮的提示框，一个有回调函数，一个没有,一个灰色按钮（禁止点击），一个蓝色(有回调)
		NotifyTwoCallOneGrayDle : function(title, msg, btnOne, btnTwo, callbackOne) {
			if(1 == isNoitfy) return;
			GenerateHtml("notifyTwoCallOneGrayDle", title, msg, btnOne, btnTwo, callbackOne);
			btnOk1(callbackOne);
			btnNo();
			document.onkeydown=function(){
				return false;
		...
		//有关闭的按钮，一个按钮的提示框，有回调函数
		NotifyOneCallClose : function(title, msg, btnOne, callbackOne) {
			if(1 == isNoitfy) return;
			GenerateHtml("notifyOneCallClose", title, msg, btnOne, callbackOne);
			btnOk1(callbackOne);
			btnNo();
			document.onkeydown=function(){
				return false;
	...
	var GenerateHtml = function (type, title, msg, btnOne, btnTwo ) {
		...
		_html += '<div class="adminNotifyTitle">' + title;
		if(type == "notifyOneCallClose") {
			_html += '<img src="http://jbyoss.oss-cn-beijing.aliyuncs.com/public/web_img/Icons/notifyClose.png" class="notifyClose">';
		}
		_html += '</div><div class="adminNotifyContent">'
		_html += '<p class="adminNotifyMsg">' + msg + '</p>';
		_html += '<p class="adminNotifyButton">';

		if (type == "notifyOne" || type == "notifyOneCall" || type == "notifyOneCallClose") {
		...
		} else if(type == "notifyTwoCallOneGrayDle") {
			_html += '<a href="javascript:;" class="adminNotifyBtn adminNotifyGray2 adminNotifyGrayDle">' + btnOne + '</a>'
			_html += '<a href="javascript:;" class="adminNotifyBtn adminNotifyBlue2" id="btn_ok1">' + btnTwo + '</a>'
		...
		if($.trim($('.adminNotifyMsg').text()).length <= 27) {
			$('.adminNotifyMsg').css({
				'padding':'60px 20px 0',
				'text-align':'center'
			})
		} else if($.trim($('.adminNotifyMsg').text()).length >= 28 && $.trim($('.adminNotifyMsg').text()).length <= 54) {
			$('.adminNotifyMsg').css({
				'padding':'50px 20px 0',
				'text-align':'left'
			})
		} else if($.trim($('.adminNotifyMsg').text()).length >= 55 && $.trim($('.adminNotifyMsg').text()).length <= 81) {
			$('.adminNotifyMsg').css({
				'padding':'40px 20px 0',
				'text-align':'left'
			})
	...
	//取消按钮事件
	var btnNo = function () {
		$("#btn_no,.notifyClose").click(function () {
			isNoitfy = 0;
		    $(".fullscr,#adminNotifyBox").remove();
			document.onkeydown=function(){
				return true;
