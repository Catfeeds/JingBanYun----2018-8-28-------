...
function simplifyText(event) {
  ...
  if (XCalc.properBrackets(input)) {
    ...
    var timer = setTimeout(function() {
      ...
      document.getElementById("result_xcalc").innerHTML = "";
      document.getElementById("result_xcalc").appendChild(graph.getCanvas());
      document.getElementById("wrapper").className="solved";
    }, 800);
  } else {
    document.getElementById("result_xcalc").innerHTML = "<div class='error'>Error: Improperly nested brackets. Remember to only use round brackets and close all opened brackets.</div>";
  }
}
