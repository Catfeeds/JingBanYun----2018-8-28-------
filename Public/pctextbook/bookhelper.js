...
function loadBookInfo() {
    ...
    else
    {
        ...
        xhr.open('GET', '{0}/Pages.xml?t={1}'.format(serverPath, new Date().getTime()), true);
        xhr.onload = function() {
            var xmlDoc = new ActiveXObject("Microsoft.XMLDOM");
            xmlDoc.async = "false";
            xmlDoc.loadXML(xhr.responseText);
            var content = $(xmlDoc).find('content');
            bookInfo.width = parseInt($(content).attr('width'));
            bookInfo.height = parseInt($(content).attr('height'));
            $(content).find('page').each(function (i, n) {
                bookInfo.pages.push({pageId: i + 1, src: $(n).attr('src')});
...
function loadPageInfo(pageId, isLeftPage) {
    ...
    isLeftPage ? currentPage1Info = {}:currentPage2Info = {};
    if(isLeftPage)
        $('#page1Wrapper').find('.actionArea,.rectClickArea,.contentClickArea,.resourceDiv').remove();
    else
        $('#page2Wrapper').find('.actionArea,.rectClickArea,.contentClickArea,.resourceDiv').remove();
    //var url = '{0}/Pages/page{1}/index.xml?t={2}'.format(serverPath, loadPageId, new Date().getTime());
    var url = '/authProxy.php?baseUrl={0}&service=eTextbook&subService=config&p={1}&t={2}'.format(serverPath, loadPageId, new Date().getTime());
    var xhr;
    if(browserName != "IE9"){
    ...
    xhr.onload = function() {
        var pageInfo = typeof(xhr.response) != 'undefined' ? xhr.response:xhr.responseText;
        var loadPage = /&p=(\d+)/.exec(url)[1];
        if(loadPage != contentPage1Id && loadPage != contentPage2Id)
            return;
        try {
            var jsonInfo = JSON.parse(pageInfo);
            if(jsonInfo['status'] == 500){
                //add mask to this page
                if(loadPage == contentPage1Id) //left page
                {
                    if($('#page1Wrapper').find('.halfMask').length == 0)
                    $('#page1Wrapper').append('<div class="halfMask"></div>')
                }
                else {
                    if($('#page2Wrapper').find('.halfMask').length == 0)
                    $('#page2Wrapper').append('<div class="halfMask"></div>')
                }
               return;
            }
        ...
        if(loadPage == contentPage1Id) //left page
        {
            $('#page1Wrapper').find('.halfMask').remove()
        }
        else {
            $('#page2Wrapper').find('.halfMask').remove()
        }
        pageInfo = pageInfo.split('&').join('& amp');
    ...
    function loadKalaokInfo() {
        ...
        var url = '/authProxy.php?baseUrl={0}&service=eTextbook&subService=subTitle&p={1}&t={2}'.format(serverPath, loadPageId, new Date().getTime());
        if (isLeftPage) {
            currentPage1KalaOKInfo = [];
        } else {
            currentPage2KalaOKInfo = []
...
function renderActionArea(nodeName, info, isLeftPage) {
    ...
    } else if(nodeName == 'AdditionalVideo' || nodeName == 'AdditionalFlash' || nodeName == 'AdditionalPPT')
    {
        ...
        var areaEle = '<img class="actionArea" id="listen_{0}" style="left:{1}px;top:{2}px;width:60px;height:60px"onclick="onClickActionArea(\'{3}\',{5})" src="{4}">'.format(info.name, positionX, positionY, encodeURI(info.path).replace("'", "####"), src, isLeftPage ? 'true' : 'false');
        ...
        var areaEle = $('<div style="position:absolute;left:{1}px;top:{2}px;" class="resourceDiv"><img class="actionArea resIcon" id="listen_{0}"style="width:60px;height:60px;position:static;" src="{4}"></div>'.format(info.name, positionX, positionY, encodeURI(info.path).replace("'", "####"), src, isLeftPage ? 'true' : 'false'));
        ...
        if($(info.resHtml).eq(0).find('wbht').length>0){
            if($(info.resHtml).eq(0).find('wbht').length==1){
                var resPath = $(info.resHtml).find('wbht').attr('path');
                var resPath = resPath.replace(/& amp/g,'&')
                var children1 = '<div class="childrenImg1"><a href="{0}" target="_black" class="wb" >互动强化</a></div>'.format(resPath,isLeftPage)
            }else {
        ...
        $('.children3Div').each(function(){
            if($(this).children('a,div').length == 2) {
                $(this).css('padding-top','5px');
                $(this).children('a,div').css('line-height','20px');
            } else if($(this).children('a,div').length == 1) {
                $(this).css('padding-top','0px');
                $(this).children('a,div').css('line-height','50px');
            } else {
                $(this).css('padding-top','0px');
                $(this).children('a,div').css('line-height','15px');
            }
        })
    }
    else{
...
function turnToContentPage() {
    ...
    if(contentsPage == undefined)
     turnPage(5, 6);
    else
     turnPage(parseInt(contentsPage), parseInt(contentsPage) + 1);
}
