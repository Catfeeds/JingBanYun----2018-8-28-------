...
function simplifyText(event) {
    ...
    if (XCalc.properBrackets(input)) {
        document.getElementById("fxwrapper").className = "";
        var timer = setTimeout(function () {
            var graph = XCalc.graphExpression(input, 300, 300);
            document.getElementById("fxresult").innerHTML = "";
            document.getElementById("fxresult").appendChild(graph.getCanvas());
            document.getElementById("fxwrapper").className = "solved";
        }, 800);
    } else {
        document.getElementById("fxresult").innerHTML = "<div class='error'></div>";
    }
}
