...
//单项文字选择题
function renderDanXiangWenZiXuanZe(question) {
    var d = [];
    d.push(getQuestionTitleHtml(question));
    ...
    $(options).each(function (i, n) {
        ...
        d.push('<div class="exerciseQuestionWordOption {4} {9}"><label><input {6} {7} data-answer="{2}" questionid="{8}" type="radio" id="q_{0}_{3}" name="q_{0}" value="{3}" {2}>{1}</label>{5}</div>'.format(question.id, n, selected, i + 1, selected, answerShow, answerMark, points, question.question_id, question.options_sort_order));
...
function renderDuoXiangWenZiXuanZe(question) {
    ...
    $(options).each(function (i, n) {
        ...
        d.push('<div class="exerciseQuestionWordOption {4} {9}"><label><input {6} {7} data-answer="{2}" questionid="{8}" type="checkbox" value="{3}" id="q_{0}_{3}" name="q_{0}" {2}>{1}</label>{5}</div>'.format(question.id, n, selected, i + 1, selected, answerShow, answerMark, points, question.question_id, question.options_sort_order));
    });
...
//判断题
function renderPanDuan(question) {
    var d = [];
    d.push(getQuestionTitleHtml(question));
    ...
    $(options).each(function (i, n) {
        ...
        d.push('<div class="exerciseQuestionWordOption {4} {9}"><label><input {6} {7} data-answer="{2}" questionid="{8}" type="radio" value="{3}" id="q_{0}_{3}" name="q_{0}" {2}>{1}</label>{5}</div>'.format(question.id, n, selected, i + 1, selected, answerShow, answerMark, points, question.question_id, question.options_sort_order));
    });
    return d.join('');
}

//问答题
function renderWenDa(question) {
    var d = [];
    d.push(getQuestionTitleHtml(question));
    d.push('<div class="textAreaInput"><textarea type="textarea" rows="5" placeholder="请输入答案" name="q_{0}" id="q_{0}" questionid="{0}"></textarea><div></div></div>'.format(question.id));
    return d.join('');

}

//填空题
function renderTianKong(question) {
    var d = [];
    d.push(getQuestionTitleHtml(question));
    ...
    $(items).each(function (i, n) {
        if (n.indexOf('#]') >= 0) {
            var answerLength = n.substring(0, n.indexOf('#]'));
            var inputWidth = answerLength.length * 25;
            innerD.push('<input type="text" questionid="{0}" id="q_{0}_{1}" name="q_{0}_{1}" style="width: {2}px" />'.format(question.id, gapsIndex, inputWidth));
...
//判断题[含多个小题]
function renderPanDuanMultiple(question) {
    var d = [];
    d.push(getQuestionTitleHtml(question));
    ...
    $(subQuestions).each(function (j, m) {
        ...
        $(options).each(function (i, n) {
            ...
            d.push('<div class="exerciseQuestionWordOption {4} {10}"><label><input {7} {9} data-answer="{2}" type="radio" questionid="{8}" id="q_{0}_{5}_{3}" name="q_{0}_{5}" value="{5}_{3}" {2}>{1}</label>{6}</div>'.format(question.id, n, selected, i + 1, selected, j, answerShow, answerMark, question.question_id, points, question.options_sort_order));
        });
    });
...
//单项选择题[含多个小题]
function renderDanXiangWenZiXuanZeMultiple(question) {
    var d = [];
    d.push(getQuestionTitleHtml(question));
    ...
    $(subQuestions).each(function (j, m) {
        //渲染题目
        d.push('<div class="radioExaminationDiv"><div class="subquestionTitle">' + $(m).find('.subquestionTitle').html() + '</div>');
        var options = $(m).find('.exercise_option');
        $(options).each(function (i, n) {
            n = $(n).html();
            var isAnswer = n.substring(0, 1) == '#';
...
function renderDuoXiangWenZiXuanZeMultiple(question) {
    ...
    $(subQuestions).each(function (j, m) {
        ...
        $(options).each(function (i, n) {
            ...
            d.push('<div class="exerciseQuestionWordOption {4} {10}"><label><input {7} {9} data-answer="{2}" questionid="{8}" type="checkbox" id="q_{0}_{5}_{3}" name="q_{0}_{5}" value="{5}_{3}" {2}>{1}</label>{6}</div>'.format(question.id, n, selected, i + 1, selected, j, answerShow, answerMark, question.question_id, points, question.options_sort_order));
        });
    });
...
//问答题[含多个小题]
function renderWenDaMultiple(question) {
    var d = [];
    d.push(getQuestionTitleHtml(question));
...
//填空题[含多个小题]
function renderTianKongMultiple(question) {
    var d = [];
    d.push(getQuestionTitleHtml(question));
    ...
    $(subQuestions).each(function (j, m) {
        ...
        $(items).each(function (i, n) {
            if (n.indexOf('#]') >= 0) {
                var answerLength = n.substring(0, n.indexOf('#]'));
                var inputWidth = answerLength.length * 25;
                innerD.push('<input type="text" id="q_{0}_{2}_{1}" name="q_{0}_{2}_{1}" questionid="{0}" style="width: {3}px" />'.format(question.id, gapsIndex, j, inputWidth));
                gapsIndex = gapsIndex + 1;
                n = n.substring(n.indexOf('#]') + 2);
            }
            innerD.push(n);
...
//渲染问题的题目（有视频的话包含视频）
function getQuestionTitleHtml(question) {
    var d = [];
    d.push('<div id="qes_{1}" class="exerciseQuestion" data-id="{1}" data-answer="" data-points="{0}" data-originalid="{2}">'.format(question.points, question.question_id, question.id));
    d.push('<span></span>');
    d.push('{0}. ({1}分) - '.format(question.question_id, question.points));
...
function renderExerciseLibraryChapter(chapterId, exercises1, callback,num) {
    $.get('index.php?m=Home&c=Admin&a=getExerciseLibraryChapterDetails', {id: chapterId,num:num}, function (res) {
        exercises = res;
        var html = [];
        $('.xiti_id').val(exercises.length);
        html.push('<div class="questionWrapper">');
        $(exercises).each(function (i, n) {

            switch (n.template_name) {
                case '单项文字选择题':
                    html.push('<div class="radioExamination">');
                    html.push(renderDanXiangWenZiXuanZe(n));
                    html.push('</div>');
                    break;
                case '多项文字选择题':
                    html.push('<div class="choiceQuestion">');
                    html.push(renderDuoXiangWenZiXuanZe(n));
                    html.push('</div>');
                    break;
                case '判断题':
                    html.push('<div class="judgmentQuestion">');
                    html.push(renderPanDuan(n));
                    html.push('</div>');
                    break;
                case '问答题':
                    html.push('<div class="answersQuestion">');
                    html.push(renderWenDa(n));
                    html.push('</div>');

                    break;
                case '填空题':
                    html.push('<tr>');  html.push('<td>');
                    html.push('<div class="completion">');
                    html.push(renderTianKong(n));
                    html.push('</div>');
                    break;
                case '判断题[含多个小题]':
                    html.push('<div class="judgmentQuestionMore">');
                    html.push(renderPanDuanMultiple(n));
                    html.push('</div>');
                    break;
                case '单项选择题[含多个小题]':
                    html.push('<div class="radioExaminationMore">');
                    html.push(renderDanXiangWenZiXuanZeMultiple(n));
                    html.push('</div>');
                    break;
                case '多项选择题[含多个小题]':
                    html.push('<div class="choiceQuestionMore">');
                    html.push(renderDuoXiangWenZiXuanZeMultiple(n));
                    html.push('</div>');
                    break;
                case '填空题[含多个小题]':
                    html.push('<div class="completionMore">');
                    html.push(renderTianKongMultiple(n));
                    html.push('</div>');
                    break;
                case '问答题[含多个小题]':
                    html.push('<div class="answersQuestionMore">');
                    html.push(renderWenDaMultiple(n));
                    html.push('</div>');
                    break;
                case '连线题':
                    html.push('<div class="matching">');
                    html.push(renderMatch(n));
                    html.push('</div>');
                    break;
            }
        });
        ...
        if(global_div==0){
            ...
            $('#exerciseWrapper_1').html(html.join(''));
            //处理连线题
            $('#exerciseWrapper_1 .matchQuestion').each(function (j, m) {
                creatMatchline($(m));
            });
        }
...
function pWidth() {
  setTimeout(function a(){
      ...
      var heightArr = new Array();
      		for(i = 0; i < $('.radioExaminationDiv').length;i++) {
      			heightArr[i] = new Array();
      			for(j = 0; j<$('.radioExaminationDiv').eq(i).children('.exerciseQuestionWordOption').length;j++) {
      				heightArr[i][j] = $('.radioExaminationDiv').eq(i).children('.exerciseQuestionWordOption').eq(j).height();
      			}
      			// alert(Math.max.apply(null, Arr[i]));//最大值
      			$('.radioExaminationDiv').eq(i).children('.exerciseQuestionWordOption').height(Math.max.apply(null, heightArr[i]))
    }
    for(var b = 0;b<$('.exerciseQuestion').length;b++){
      for(var l=0 ;l<$('.exerciseQuestion').eq(b).children('p').find('img').length;l++){
          if($('.exerciseQuestion').eq(b).children('p').find('img').eq(l).width() > '500' ||$('.exerciseQuestion').eq(b).children('p').find('img').eq(l).width() > '400'){
            console.log(l)
              $('.exerciseQuestion').eq(b).children('p').find('img').eq(l).css('display','block').after('<br>')
          }
      }
   }
 },1000)
...
/*function createExerciseLibraryChapter(homework_id, exercises1, callback) {
 $.getJSON('index.php?m=Home&c=Teach&a=getHomeworkInfo', {id: homework_id}, function (res) {
 exercises = res;
 var html = [];
 html.push('<div class="questionWrapper">');
 $(exercises).each(function (i, n) {
