...
function getQuestionTitleHtml_1(question) {
    ...
    d.push('<div id="qes_{1}" class="exerciseQuestion" data-id="{1}" data-answer="" data-points="{0}" data-originalid="{2}">'.format(question.points, global_I,question.id));//question.question_id
	if(isPrint == 1)
		d.push('<input type="checkbox" class="selectSingleExercises" checked>');
    d.push('<span>{0}.</span> ({1}分) - '.format(global_I, question.points));
...
function renderMatch_1(question) {      
	...
	isPrint = 0;
    $.getJSON('index.php?m=Home&c=Teach&a=getHomeworkInfo', {id: homework_id,'student_id':student_id}, function (res) {
        renderExercises(res);
	...
	//根据习题ID列表获取习题
	function createExerciseByIdString(idString, callback) {
	isPrint = 1;
    $.getJSON('index.php?m=Home&c=Teach&a=getExercisesByIds', {exercises_id: idString}, function (res) {
        renderExercises(res);
        if (typeof callback == 'function') {
            callback();
        }
