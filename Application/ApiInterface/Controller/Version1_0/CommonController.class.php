...
class CommonController extends PublicController
{
     ...
     * @参数：machine_type[string] Y 用户机器字符串
     * @返回值：array(
     *    status 状态码
     *    message 异常字符串
     *    result 结果数组
    ...
    public function getPersonalInfo()
    {
        ...
        if((($role==1) || ($role==0))&& (!empty($result->school_id)))//增加学校昵称
        {
        ...
        if (preg_match('/Resources/', $result->avatar)){
            ...
            $result['img_url'] = 'http://'.WEB_URL.'/Uploads/'.$localAvatarFolder.'/'.$result['avatar'];
        }
    ...
    public function resetPassword()
    {
        ...
        if(empty(D('UserInfo')->getUserInfoBytelephone($telephone)))
        ...
        if ($_FILES["file"]["type"] == "image/jpeg") {
            if ($_FILES["file"]["error"] > 0) {
                ...
                if( $jsonurl ) {
                    ...
                    $this->ajaxReturn(array('status' => 200, 'message' => '上传失败'));
                }
            }
        } else {
            $this->ajaxReturn(array('status' => 200, 'message' => '图片格式不正确'));
        }
    }
