...
{ 
    ...
    public function delClassData(){
        ...
        if(empty($classId)) {
            $result['code'] = 400;
            $this->ajaxReturn(array('status' => 406, 'message'=>'error','result' => $result));
        }
    ...
    public function teachClassList() {
        ...
        $check['biz_class.class_status'] =  2;
        ...
        $teach_info['avatar']=C('oss_path').$teach_info['avatar'];
        $big_data['teach_info']=$teach_info;
        $big_data['class_data']=$result;
        $this->ajaxReturn(array('status' => 200, 'message'=>'success','result' => $big_data));
    }
    ...
    public function classStudent(){
        ...
        if(empty($class_result)){
            $this->ajaxReturn(array('status' => 406, 'message'=>'班级信息不存在'));
        }
        ...
        if(empty($student_result)){
            ...
            $this->ajaxReturn(array('status' => 406, 'message'=>'该班级下还没有学生','result' => $big_data));
        }else{
            $student_ids='';
            ...
            if(empty($student_info)){
                ...
                foreach($student_info as $key=>$val){
                    ...
                    if (preg_match('/Resources/', $val['avatar'])){
                        ...
                        $student_info[$key]['avatar']='http://'.WEB_URL.'/Uploads/StudentAvatars/'.$val['id'].'.jpg';
                    }
                }
    ...
    public function studentClassList(){
        ...
        if(empty($class_student)){
            $this->ajaxReturn(array('status' => 406, 'message'=>'您还没有加入任何班级'));
        }else{
            $class_model=D('Biz_class');
            $class_ids='';
            ...
            sort($class_result);
            $this->ajaxReturn(array('status' => 200, 'message'=>'success','result' => $class_result) );
        }
    }
    ...
    public function parentClassList(){
        ...
        if(empty($student_info)){
            $this->ajaxReturn(array('status' => 406, 'message'=>'您还没有添加任何小孩') );
        }
        $student_ids='';
        ...
        if(empty($student_class)){
            $this->ajaxReturn(array('status' => 406, 'message'=>'您的小孩还没有加入任何班级') );
        }else{
            //这里去得到年级,学校
            /*
            ...
            $this->ajaxReturn(array('status' => 200, 'message'=>'success','result' => $student_class) );
        }
    }
    ...
    public function updateStudentParent(){
        ...
        if($student_model->updateStudentParentInfo($parent_id,$student_id)){
            ...
            $controller_obj->addPushUserMessage('ADDCHILD',4,$parent_id,$parameter_arr); ////
            
            $this->ajaxReturn(array('status' => 200, 'message'=>'success') );
        }else{
            $this->ajaxReturn(array('status' => 406, 'message'=>'添加失败') );
        }
    ...
    public function classTimeatable(){
        ...
        $id=intval(I('classId'));
        if(!$id){
            $this->ajaxReturn(array('status' => 406, 'message'=>'failed') );
        }
        $class_model=D('Biz_class');
