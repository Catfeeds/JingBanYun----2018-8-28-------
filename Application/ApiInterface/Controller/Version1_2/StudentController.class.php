...
class StudentController extends PublicController
{
    ...
    public function login()
    {
        ...
        if ($userInfo['password'] == $password) {
            ...
            (new CommonController())->getAvatarInfo(ROLE_STUDENT,$result);
    ...
    public function register()
    {
        ...
        $password  = sha1(getParameter('pwd', 'str'));
        $birthday = getParameter('birthday', 'str',false);
...
//        }
    ...
    public function modifyInfo()
    {
        ...
        if(!empty($birthday)) {
            ...
            if ($birthdayTime == 0)
                $this->ajaxReturn(array('status' => 403, 'message' => '出生日期格式不正确'));
        }
        ...
        $oldStudentInfo = D('Auth_student')->getStudentInfo($id);

        $studentInfo = D('Auth_student')->getStudentInfoByTelAndName($oldStudentInfo['parent_tel'],$name);
        if($studentInfo['id'] != $id)
        {
            $this->ajaxReturn(array('status' => 407, 'message' => '该家长下已经有相同姓名的学生'));
        }
        ...
        if($this->Model->updateInfoById($data,$id)) {
            ...
            if (!empty($data['school_id']) && ($oldStudentInfo['school_id'] != $data['school_id'])) {
                //修改学生学校审核状态为未审核
                ...
                if ($school_info['is_create_administartor'] == 1) { //修改后的学校为认证的学校
                    ...
                    D('UserInfo')->sendMsg(ROLE_STUDENT,$id,json_encode($parameters),'XUEXIAO_ZHENG',date('Y-m-d H:i:s',time()+3));
                    //$controller_obj->addPushUserMessage('XUEXIAO_ZHENG', ROLE_STUDENT, $id, $parameters);
                }
            }
            ...
            D('UserInfo')->sendMsg(ROLE_STUDENT,$id,json_encode($parameter_arr),'INFO_MODIFIED_BYSELF_STUDENT',date('Y-m-d H:i:s',time()+3));
            //$controller_obj->addPushUserMessage('INFO_MODIFIED_BYSELF_STUDENT', ROLE_STUDENT, $id, $parameter_arr);
