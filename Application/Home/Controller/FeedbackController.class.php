...
class FeedbackController extends PublicController{
	...
	public function addfeedback() {
        if ( empty($_SESSION['num']) || $_SESSION['num']==0 ) {
            $_SESSION['num'] = 0;
        }
        ...
        foreach ($node_list as $key => $value) {
            $data['fid'] = $value['id'];
            $row = M('account_node_list')->where( $data )->field('id,node_name,fid')->order('order_id desc')->select();
	...
	public function savefeedback() {
		...
		if (!empty($student_session)) {
            ...
            if (empty($contact)) {
                $contact = session('student.parent_tel');
            }
		...
		if (!empty($teacher_session) && session('teacher')  != 'youke') {
            ...
            if (empty($contact)) {
                $contact = session('teacher.telephone');
            }
		...
		if (!empty($parent_session)) {
            ...
            if (empty($contact)) {
                $contact = session('parent.telephone');
            }
    ...
    public function addUserFeed() {
        ...
        foreach ($list as $key => $value) {
            ...
            if (!empty($value['user_id']) && !empty($value['user_role'])) {
                ...
                $savemap['id'] = $value['id'];
                $savedata['contact'] = $row['tel'];
                M('feedback')->where( $savemap )->save( $savedata );
            }
