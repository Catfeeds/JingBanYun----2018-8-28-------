...
class MessageController extends Controller
{
    ...
    //发送访客消息
    public function sendVisitorNews() {
        if (!session('?admin')) redirect(U('Login/login'));
        $where=[];
        $userstatus = I('userstatus');
        $set_day = I('set_day');

        $userstatus = empty($userstatus)?1:$userstatus;
        $set_day = empty($set_day)?1:$set_day;
        ...
        $count = M('log_count_new')->where( $where )->order('access_time desc')->count();

        $Page = new \Think\Page($count, C('PAGE_SIZE_FRONT'));

        $show = $Page->show();
        $list = M('log_count_new')->where( $where )->limit($Page->firstRow . ',' . $Page->listRows)->order('access_time desc')->select();

        foreach ($list as $k=>&$v) {
            if ($v['role'] ==0 ){ //教师
                $v['role_name'] = "教师";
                $info = M('auth_teacher')->field("auth_teacher.*,dict_schoollist.school_name")->join("left join dict_schoollist ON dict_schoollist.id=auth_teacher.school_id")->where("auth_teacher.id=".$v['user_id'])->find();

                $v['name'] = $info['name'];
                $v['telephone'] = $info['telephone'];
                $v['school_name'] = $info['school_name'];

                if (!empty($info['course_id'])) {
                    $dict_course = M('dict_course')->where("id=".$info['course_id'])->find();
                    $v['course_name'] = $dict_course['course_name'];
                }

                if (!empty($info['grade_id'])) {
                    $grade_list = M('auth_teacher_second')->join("left join dict_grade ON dict_grade.id=auth_teacher_second.grade_id")->where("auth_teacher_second.grade_id=".$info['grade_id']." and teacher_id=".$v['user_id'])->group('auth_teacher_second.grade_id')->select();
                    $grade_list_data = "";

                    if(!empty($grade_list)) {
                        foreach ($grade_list as $kk=>$vv) {
                            if ($kk==0) {
                                $grade_list_data = $vv['grade'];
                            } else {
                                $grade_list_data .= ','.$vv['grade'];
                            }
                        }
                    }
                    $v['grade_name'] = $grade_list_data;
                }


            } elseif($v['role'] ==1) { //学生
                $v['role_name'] = "学生";
                $info = M('auth_student')->join("left join dict_grade ON dict_grade.id=auth_student.grade_id")->join("left join dict_schoollist ON dict_schoollist.id=auth_student.school_id")->where("auth_student.id=".$v['user_id'])->find();
                $v['name'] = $info['student_name'];
                $v['telephone'] = $info['parent_tel'];
                $v['school_name'] = $info['school_name'];
                $v['grade_name'] = $info['grade'];
                ...
                $v['role_name'] = "家长";
                $info = M('auth_parent')->join("left join dict_grade ON dict_grade.id=auth_parent.grade_id")->join("left join dict_schoollist ON dict_schoollist.id=auth_parent.school_id")->where("auth_parent.id=".$v['user_id'])->find();
                $v['name'] = $info['parent_name'];
                $v['telephone'] = $info['telephone'];
                $v['school_name'] = $info['school_name'];
                $v['grade_name'] = $info['grade'];
                if (!empty($info['course_id'])) {
    ...
    //发送推送消息
    public function sendVisitorNewsData() {
        if (!session('?admin')) redirect(U('Login/login'));
        $content = I('content'); //要发送的内容
        $message_id = I('message_id'); //要发送的id

        if (strpos($message_id,',')!== false) { //推送多个
            $teacher_id=[];
            $student_id=[];
            $parent_id=[];
            ...
            if (!empty($teacher_id)) {
                ...
                $controller_obj=new \Home\Controller\MessageController(); //老师
                $controller_obj->addPushUserMessage('IMPORTANT_UPDATE',2,implode(',',$teacher_id),$parameter_arr);
            }
            ...
            if (!empty($student_id)) {
                ...
                $controller_obj=new \Home\Controller\MessageController(); //学生
                $controller_obj->addPushUserMessage('IMPORTANT_UPDATE',3,implode(',',$student_id),$parameter_arr);
            }
            ...
            if (!empty($parent_id)) {
                ...
                $controller_obj=new \Home\Controller\MessageController(); //家长
                $controller_obj->addPushUserMessage('IMPORTANT_UPDATE',4,implode(',',$parent_id),$parameter_arr);
            }
            ...
            if ($message_id[0]==0) {
                ...
                $message_id = explode('_',$message_id);
                $controller_obj=new \Home\Controller\MessageController(); //老师
                $controller_obj->addPushUserMessage('IMPORTANT_UPDATE',2,$message_id[1],$parameter_arr);
            }
            ...
            if ($message_id[0]==1) {
                ...
                $controller_obj=new \Home\Controller\MessageController(); //学生
                $controller_obj->addPushUserMessage('IMPORTANT_UPDATE',3,$message_id[1],$parameter_arr);
            }
            ...
            if ($message_id[0]==2) {
                ...
                $controller_obj=new \Home\Controller\MessageController(); //家长
                $controller_obj->addPushUserMessage('IMPORTANT_UPDATE',4,$message_id[1],$parameter_arr);
            }
            $setdata['code'] = 200;
            echo json_encode($setdata);
        }
     ...
     * 发送一条消息
     */
    public function sendMessage($messageId = '', $messageUrl = '', $pageCategory = 2)
    {
        //if (!session('?admin')) redirect(U('Login/login'));
        if (empty($messageId)) {
        ...
        switch ($messageCategory) {
            case MESSAGE_APPPUSH:
                $userInfos = $this->model->getUserChannelWithUnReadCountMachineType($messageId);
                $bSend = batchSendPushUserDevice($this->messageKey, array('title' => $sendMsg, 'description' => ' '), $userInfos, array('url' => $messageUrl, 'category' => $pageCategory));
                break;
                ...
                if (!empty($teaids)) {
                    ...
                    if (is_array($tids)) {
                        ...
                        foreach ($tids as $tk => $tv) {
                            ...
                            $teacherdataList[] = $teachermap;
                        }

                        $this->model->pushAddUserAll($teacherdataList);
                    } else {
            ...
            if ($is_all_stu == "true" && $is_all_select_stu == 1) { //所有的学生都发
                ...
                if (!empty($stuids)) {
                    ...
                    if (is_array($sids)) {
                        ...
                        $this->model->pushAddUserAll($studataList);
                ...
                if (!empty($parentids)) {
                    ...
                    if (is_array($pids)) {
                        ...
                        foreach ($pids as $pk => $pv) {
                            ...
                            $parentdataList[] = $parentmap;
                        }

                        $this->model->pushAddUser($parentdataList);
                    } else {
                        $parentmap = array(
                            'message_id' => $message_id,
                            'role_id' => 4,
            ...
            if ($is_all_parent == "true" && $is_all_select_parent == 1) { //所有的家长都发
                ...
                if (!empty($parentids)) {
                    ...
                    if (is_array($pids)) {
                        ...
                        $this->model->pushAddUserAll($parentdataList);
                    } else {
                        $parentmap = array(
                            'message_id' => $message_id,
                            'role_id' => 4,
    ...
    public function addPushUserMessage($category, $role, $userId, $parameters)
    {
        ...
        $message_title = substr($message_content, 0, 200);
        ...
        return $this->sendMessage($messageId, $messageUrl, $messageCategory);
    }
     ...
     * 显示未读管理员消息
     */
    public function displayUnreadAdminMessage(){
       $id =  getParameter('id','int');
       $result = session('admin.message')[$id]['content'];
       $_SESSION['admin']['message'][$id]['hasRead'] = 1;
        //设置已读
       $this->showMessage(200 , 'success', $result);
    }
