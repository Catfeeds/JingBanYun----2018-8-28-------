...
function think_filter(&$value,$fieldName,$isSqlFilter){    
    // TODO 其他安全过滤    
    if($isSqlFilter==1){    
        $value=sql_filter($fieldName,$value);
    }       
    
    /*// 过滤查询特殊字符
    if(preg_match('/(EXP|NEQ|GT|EGT|LT|ELT|OR|XOR|LIKE|NOTLIKE|NOT BETWEEN|NOTBETWEEN|BETWEEN|NOTIN|NOT IN|IN)/is',$value)){ 
        //如果整数的查询,没有转整数等就会有注入
        $value .= ' '; 
    }*/
}
...
function getParameter($fieldNameList, $className,$isRequired=true,$isSqlFilter=1)
{
    if ('' == (I($fieldNameList)) && $isRequired)
    {   
        echo json_encode(array('status' => 500, 'message' => '缺少' . $fieldNameList . '参数'));
        exit;
    }   
    ...
    switch ($className) {
        case 'int': 
            $value = I($fieldNameList) === null ? null : intval(I($fieldNameList));
            if($value === null)
                return false;
            think_filter($value,$fieldNameList,$isSqlFilter);   
            return $value;
        case 'str':
            $value = I($fieldNameList) === null ? null : I($fieldNameList);
            if($value === null)
                return false;
            $value = remove_xss(I($fieldNameList));
            ...
            return $value;
        case 'iArr':
            $value = I($fieldNameList);
            foreach($value as $key=>$val)
            {
                $value[$key] = intval(stripslashes(htmlspecialchars_decode($value[$key])));
            }
            return $value;
        case 'sArr':
            $value = I($fieldNameList);
            foreach($value as $key=>$val)
            {
                $value[$key] = stripslashes(htmlspecialchars_decode(remove_xss($value[$key])));
                    if(!get_magic_quotes_gpc()) { 
...
//这里现在什么都没做,即使这里把关键字过滤了
function sql_filter($key,$value){   
    $getfilter="'|(and|or)\\b.+?(>|<|=|in|like)|\\/\\*.+?\\*\\/|<\\s*script\\b|\\bEXEC\\b|UNION.+?SELECT|UPDATE.+?SET|INSERT\\s+INTO.+?VALUES|(SELECT|DELETE).+?FROM|(CREATE|ALTER|DROP|TRUNCATE)\\s+(TABLE|DATABASE)";
    $postfilter="\\b(and|or)\\b.{1,6}?(=|>|<|\\bin\\b|\\blike\\b)|\\/\\*.+?\\*\\/|<\\s*script\\b|\\bEXEC\\b|UNION.+?SELECT|UPDATE.+?SET|INSERT\\s+INTO.+?VALUES|(SELECT|DELETE).+?FROM|(CREATE|ALTER|DROP|TRUNCATE)\\s+(TABLE|DATABASE)";
        
    
    if($_SERVER["REQUEST_METHOD"]=='POST'){
        return sql_check($key,$value,$postfilter);
    }else{
        return sql_check($key,$value,$getfilter);
    }
    
}

function sql_check($strFiltKey,$strFiltValue,$arrFiltReq){             
    if(is_array($strFiltValue))
    {
        $strFiltValue=implode($strFiltValue);
    }     
    
    if (preg_match("/".$arrFiltReq."/is",$strFiltValue,$data)){          
        writelog("<br><br>操作IP: ".get_client_ip()."<br>操作时间: ".strftime("%Y-%m-%d %H:%M:%S")."<br>页面地址:".$_SERVER["REQUEST_URI"]."<br>提交方式: ".$_SERVER["REQUEST_METHOD"]."<br>提交参数: ".$strFiltKey."<br>提交数据: ".$strFiltValue);
        $strFiltValue=str_replace($data[0], ' ', $strFiltValue);   
    }
    return $strFiltValue;
}
...
/**
 * 获取IP对应省份
 * @param integer $queryIP IP字符串
 * @return 中文省份
 */
function getIPLoc_sina($queryIP){
    $url = 'http://api.map.baidu.com/location/ip?ak=F454f8a5efe5e577997931cc01de3974&ip='.$queryIP;
    $ch = curl_init($url);
    ...
    if (1) {
        $loc = $location->content->address_detail->province;
    }
    return $loc;
}
...
function curl($action, $params=[],$method='get',$useCustomRequest=0) {
    $httpHeader = createHttpHeader();
    $ch = curl_init();
    $method = strtoupper($method);
    if($useCustomRequest == 1 || $method != "POST")
