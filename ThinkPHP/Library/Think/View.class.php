...
class View {
     ...
     * no cache
     */
    public function display_nocache($templateFile='',$charset='',$contentType='',$content='',$prefix='') {
        G('viewStartTime');
        // 视图开始标签
        Hook::listen('view_begin',$templateFile);
        // 解析并获取模板内容
        $content = $this->fetch($templateFile,$content,$prefix);
        // 输出模板内容
        $this->render($content,$charset,$contentType,'no-cache, must-revalidate,no-store');
        // 视图结束标签
        Hook::listen('view_end');
    }
    /**
     ...
     * @return mixed
     */
    private function render($content,$charset='',$contentType='',$cacheoption=''){
        ...
        if($cacheoption=='')
        {
        $cacheoption = C('HTTP_CACHE_CONTROL');
        }
        header('Cache-control: '.$cacheoption);  // 页面缓存控制
